...
ASTUBaseCharacter::ASTUBaseCharacter(const FObjectInitializer& ObjInit)
  ...
  SpringArmComponent->SocketOffset = FVector(75.0, 60.0, 60.0);

  CameraComponent = CreateDefaultSubobject<UCameraComponent>("CameraComponent");
  CameraComponent->SetupAttachment(SpringArmComponent);

  HealthComponent = CreateDefaultSubobject<USTUHealthComponent>(
      "HealthComponent");
...
void ASTUBaseCharacter::BeginPlay() {
  ...
  HealthComponent->OnHealthChanged.AddUObject(
      this, &ASTUBaseCharacter::OnHealthChanged);
...
void ASTUBaseCharacter::SetupPlayerInputComponent(
  ...
  PlayerInputComponent->BindAxis("LookUp", this,
                                 &ASTUBaseCharacter::AddControllerPitchInput);
  PlayerInputComponent->BindAxis("TurnAround", this,
                                 &ASTUBaseCharacter::AddControllerYawInput);
  PlayerInputComponent->BindAction("Jump", IE_Pressed, this,
                                   &ASTUBaseCharacter::Jump);
  PlayerInputComponent->BindAction("Run", IE_Pressed, this,
                                   &ASTUBaseCharacter::OnStartRunning);
  PlayerInputComponent->BindAction("Run", IE_Released, this,
                                   &ASTUBaseCharacter::OnStopRunning);
}

bool ASTUBaseCharacter::IsRunning() const {
  return IsMovingForward && IsPlayerWantsToRun && !GetVelocity().IsZero();
}
...
void ASTUBaseCharacter::OnStartRunning() {
  IsPlayerWantsToRun = true;
}

void ASTUBaseCharacter::OnStopRunning() {
  IsPlayerWantsToRun = false;
}

void ASTUBaseCharacter::OnDeath() {
  UE_LOG(BaseCharacterLog, Warning, TEXT("Player is dead"))
...
void ASTUBaseCharacter::OnGroundLanded(const FHitResult& Hit) {
  ...
  UE_LOG(BaseCharacterLog, Warning, TEXT("Fall Vel: %f"), FallVelocity);

  if (FallVelocity < LandedDamageVelocity.X) return;
  const auto FinalDamage = FMath::GetMappedRangeValueClamped(
      LandedDamageVelocity, LandedDamage, FallVelocity);
